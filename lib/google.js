...
export async function getUnreadEmails(accessToken, refreshToken) {
  ...
  const query  = 'is:unread newer_than:2d (from:MVS OR from:Opteon OR from:UWM OR from:Pennymac OR from:AppraisalFlo OR subject:appraisal OR subject:order OR subject:inspection)';
...
export async function getRevisions(accessToken, refreshToken) {
  ...
  // Exclude Label_8 (INBOX/Corrections) — already handled
  const query = '(subject:revision OR subject:revise) -label:Label_8';
