...
export default function Home() {
  ...
  // Research a single address client-side — no timeout
  const researchAddress = useCallback(async (item) => {
    try {
      const res  = await fetch('/api/research', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ address: item.address, timeStr: item.timeStr }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Research failed');
      return json;
    } catch (e) {
      return { address: item.address, timeStr: item.timeStr, _error: true };
    }
  }, []);
