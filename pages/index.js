...
function Tag({ label, color }) {
  return <span className="tag" style={{ background: `${color}18`, color, border: `1px solid ${color}30` }}>{label}</span>;
}
...
function SectionCard({ icon, title, sub, borderColor, delay, children }) {
  const style = borderColor
    ? { borderLeft: `3px solid ${borderColor}`, border: `1px solid ${borderColor}30` }
    : { border: '1px solid var(--border)' };
  return (
    <div className={`fade-up ${delay}`} style={{ background: 'var(--bg2)', borderRadius: 4, padding: '20px 22px', marginBottom: 16, ...style }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 14 }}>
        <span style={{ fontSize: '1.1rem' }}>{icon}</span>
        <div>
          <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700, fontSize: '1.05rem', color: 'var(--accent)' }}>{title}</div>
          {sub && <div style={{ fontSize: '0.62rem', color: 'var(--faint)', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: 2 }}>{sub}</div>}
        </div>
      </div>
      {children}
    </div>
...
function RevisionSection({ items }) {
  ...
  function ageColor(d) {
    ...
    return n >= 5 ? 'var(--red)' : n >= 2 ? 'var(--accent)' : 'var(--dim)';
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={{ fontSize: '0.62rem', color: 'var(--faint)', marginBottom: 8 }}>
      ...
      {items.map((r, i) => (
        <div key={i} className="email-row" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8, borderLeft: `2px solid ${ageColor(r.daysAgo)}`, paddingLeft: 10 }}>
          <div>
            ...
            <div style={{ fontSize: '0.68rem', color: 'var(--faint)', lineHeight: 1.6 }}>{r.description || r.snippet}</div>
          </div>
          <div style={{ fontSize: '0.6rem', color: 'var(--faint)', whiteSpace: 'nowrap' }}>{new Date(r.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</div>
        </div>
      ))}
    </div>
  );
      ...
      {emails.map((e, i) => (
        <div key={i} className="email-row" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8 }}>
          <div>
            ...
            <div style={{ fontSize: '0.68rem', color: 'var(--faint)', lineHeight: 1.6 }}>{e.summary || e.snippet}</div>
          </div>
          <div style={{ fontSize: '0.6rem', color: 'var(--faint)', whiteSpace: 'nowrap' }}>{e.date ? new Date(e.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : ''}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 4, padding: '16px 18px', marginBottom: 14 }}>
      ...
      {p.flags?.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          ...
          {p.flags.map((f, fi) => <div key={fi} style={{ fontSize: '0.68rem', color: 'var(--accent)', lineHeight: 1.6 }}>· {f}</div>)}
        </div>
      )}
      {p.pullManually?.length > 0 && (
        <div style={{ background: 'rgba(68,68,85,0.1)', border: '1px solid var(--border)', borderRadius: 3, padding: '8px 12px' }}>
          ...
          {p.pullManually.map((item, pi) => <div key={pi} style={{ fontSize: '0.65rem', color: 'var(--dim)' }}>· {item}</div>)}
        </div>
      )}
    </div>
  );
...
function PropertySection({ properties }) {
  if (!properties?.length) return <Empty msg="No appraisal appointments found for tomorrow." />;
      ...
      {STEPS.map((s, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            ...
            <div style={{ fontSize: '0.72rem', color: done ? 'var(--green)' : active ? 'var(--accent)' : 'var(--faint)' }}>{s}</div>
          </div>
        );
      })}
    </div>
...
export default function Home() {
  ...
  async function runBriefing() {
    ...
    const stepDelay = (s) => { setStep(s); return new Promise(r => setTimeout(r, 800)); };
    const walkSteps = async () => {
      for (const s of STEPS.slice(0, -1)) await stepDelay(s);
    };
    walkSteps();
  ...
  if (status === 'loading' || status === 'unauthenticated') {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: '0.72rem', color: 'var(--faint)' }}>Loading...</div>
      </div>
    );
  }
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: '32px 24px 64px', maxWidth: 820, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 36, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: '0.62rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--faint)', marginBottom: 8 }}>Simmons Appraisals LLC</div>
          ...
          <button
            ...
            style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '6px 14px', background: 'none', color: 'var(--faint)', border: '1px solid var(--border)', borderRadius: 3, cursor: 'pointer', marginTop: 4 }}
          >
            Sign Out
          </button>
        </div>
        ...
        {state === 'idle' && (
          <button className="run-btn" onClick={runBriefing} style={{
            fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700, fontSize: '0.95rem', letterSpacing: '0.04em',
            padding: '16px 44px', background: 'var(--accent)', color: 'var(--bg)', border: 'none', borderRadius: 4, marginBottom: 32,
          }}>
        ...
        {state === 'error' && (
          <div style={{ padding: '16px 18px', background: 'rgba(224,112,112,0.07)', border: '1px solid rgba(224,112,112,0.3)', borderRadius: 4, marginBottom: 24 }}>
            ...
            <button className="run-btn" onClick={runBriefing} style={{ fontFamily: "'DM Mono',monospace", fontSize: '0.68rem', padding: '8px 20px', background: 'var(--bg3)', color: 'var(--dim)', border: '1px solid var(--border)', borderRadius: 3 }}>Retry</button>
          </div>
        )}
        ...
        {state === 'done' && data && (
          <div>
            ...
            <SectionCard icon="⚡" title="Today's Action Items" sub="Priority-ranked tasks" borderColor="var(--accent)" delay="s1">
              <ActionSection items={data.actions} />
            </SectionCard>

            <SectionCard icon="✏️" title="Outstanding Revisions" sub="All unresolved — Corrections folder excluded" borderColor="var(--red)" delay="s2">
              <RevisionSection items={data.revisions} />
            </SectionCard>

            <SectionCard icon="✉" title="AMC & Client Emails" sub="Last 48 hours — unread" delay="s3">
              <EmailSection emails={data.emails} />
            </SectionCard>

            <SectionCard icon="📅" title="Schedule" sub="Today & tomorrow" delay="s4">
              <CalendarSection events={data.calendar} />
            </SectionCard>

            <SectionCard icon="🏠" title="Pre-Inspection Property Research" sub="Tomorrow's appointments — auto-researched" borderColor="var(--accent2)" delay="s5">
              <PropertySection properties={data.properties} />
            </SectionCard>

            <SectionCard icon="📰" title="Industry News" sub="Appraisal & mortgage — last 7 days" delay="s6">
